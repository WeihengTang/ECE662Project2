...
\end{enumerate}
Implementation: \texttt{olt/solver.py}, gradient scaled by $1/N$.
\end{frame}
...
+ joint OLT + NS & $0.9850$ & $\mathbf{0.8811}$ \\
\bottomrule
\end{tabular}
\hfill
\includegraphics[width=0.42\linewidth]{part2/mnist_frozen_vs_olt.png}\\[0.4em]
{\small $-0.7$ on clean, \textbf{$+1.7$ on rotated} -- modality-invariant
...
\item Differentiable through \texttt{torch.linalg.svd}.
...
\end{itemize}
\item Limitation: SVDs per batch (modest cost), and very large
$\lambda$ overpowers cross-entropy.
\end{itemize}
\end{frame}
