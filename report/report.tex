...
exactly the within-subspace structure that OLT preserves. In
contrast, LDA explicitly collapses each class to its centroid in the
projected space, making nearest-centroid a natural companion. The
lesson is that OLT \emph{requires} a subspace-aware classifier (such
as nearest-subspace) to realise its benefit; a point-based rule
(nearest centroid) negates its geometric prior.
